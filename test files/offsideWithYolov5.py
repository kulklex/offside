...
class OffsideDetector:
    ...
    def detect_offside(self, detections, frame):
        ...
        ball_detections = detections[detections['class'] == 'ball']
        ...
        # Always assume ball is being played (per user instruction)
        if ball_visible:
...
def draw_definitive_results(frame, detections, offside_players, detector):
    ...
    for _, player in players.iterrows():
        ...
        cv2.putText(frame, "OFF" if is_offside else "DEF", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
...
for image_path in input_folder.glob('*.*'):
    ...
    results.render()
    image_with_boxes = results.ims[0]
    image_with_offside = draw_definitive_results(image_with_boxes, detections, offside_players, detector)

    save_path = output_folder / image_path.name
    cv2.imwrite(str(save_path), image_with_offside)

    print(f"Offside decision: {'OFFSIDE' if offside_players else 'NO OFFSIDE'}")
    if offside_players:
        print(f"Offside players: {len(offside_players)}")
